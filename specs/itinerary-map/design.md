...
- **Markers are numbered circles, no name labels anywhere**: each marker (`{lat, lng, color, number?, zIndex?}`) is a single `ol.style.Style` with an `ol.style.Circle` image (radius `2.5 * scale`, white `0.4 * scale` stroke) and, if `number` is set, a centered `ol.style.Text` (bold, white, `4 * scale`px) showing that number. Shrunk repeatedly on request against a real densely-packed itinerary — radius/stroke/font: `10`/`2`/`11px` → `6`/`1.5`/`8px` ("still huge") → `3.5`/`1`/`5px` ("a bit smaller still") → `2.5`/`0.75`/`4px` → stroke to `0.4` ("even thinner border") → font to `3px` ("1pt smaller still"). Name labels (a separate `Text` style with a white pill background, positioned beside the marker) existed in an earlier revision but were removed entirely (see below) — every marker everywhere is numbers-only now; full names are always in the text next to the map instead, never on the map image itself.
...
- **`marker.zIndex` — required, not cosmetic**: a real export showed **zero visible stop markers** on the overview, despite the route line (which only draws between stops) clearly being present — the stop circles were there but completely hidden under POI circles rendered on top of them at nearly the same coordinates. Root cause: OpenLayers vector layers do **not** guarantee features render in insertion order — its internal spatial index can visit overlapping features in either order, so "stops added to the array first" was never a real guarantee of draw order. `ol.style.Style#zIndex` is OL's actual, documented mechanism for controlling this. Stops get `zIndex: 2`; the route line gets `zIndex: 1.5`; POIs get `zIndex: 1 + poiNumberByRef.get(p) / 100000` — a small, unique fractional offset per POI (not a flat `1` for every one of them), because two POIs sharing the same zIndex have no more guaranteed order *between themselves* than the original stop-vs-POI bug did (same root cause, one tier down) — a real, if lower-frequency, complaint about numbers seeming to bleed through neighboring circles turned out to trace back to this. **Verified separately** that zIndex-based hiding itself works correctly once two features' zIndex actually differ: an isolated two-marker test with a precise 15px pixel offset (computed via `view.getResolution()`, not eyeballed) showed the higher-zIndex marker's number pixels present and the lower one's completely absent (0 leaked pixels) — so the fix here is purely about giving every POI comparison a real, stable winner, not about text somehow painting over a covering circle.
  - **Bug found in the field — a leg map made it look like the route line ran from a POI (green) to a stop (blue)**: the line had no explicit `zIndex` (defaulting to `0`), the lowest of anything on the map. Real coordinates aren't evenly spread out — a POI can sit right on, or very close to, the straight path between two stops (e.g. a landmark at a ferry crossing point). Wherever that happened, the line's own dashes disappeared behind that POI's circle (zIndex ~1.0–1.01 at the time, still above the line's `0`), so the line visually appeared to terminate at the green POI instead of visibly continuing past it to the blue stop. **Fix**: gave the line `zIndex: 1.5` — above every POI (whose zIndex now stays comfortably below that, see the `/ 100000` divisor above, tightened from `/ 1000` purely to keep that headroom safe at any realistic POI count) but below stop markers (`2`) — so the dashed line now visibly draws over/through nearby POI markers rather than disappearing behind them, making clear it's one continuous stop-to-stop route regardless of what it happens to pass near.
- **Map images respect the page's side margins**: `addMapImage()` sizes and places the rendered image within `margin`/`contentWidth`, same as every text element in this document. (Earlier revision deliberately bled maps to the full page width — `0` to `pageWidth` — for bigger, less-crowded markers; reverted on request in favor of a consistent side margin throughout the document.)
- **`buildPdf()`** produces the `jsPDF` document: itinerary title, an overview map image (all stops + all POIs — both numbered, connected by the route line, no names), then per stop (in the same order/grouping as `computeStoryGroups()`) — name/place label, type, dates, flags, booking, nested numbered POIs — followed by that leg's map image (two connected, numbered stop markers + that leg's numbered POIs — no names on either). Text content mirrors `buildMarkdown()`'s wording and field selection (no Notes, no price — see above; note `buildMarkdown()` itself was **not** changed to add POI numbers — this numbering scheme exists specifically to compensate for the PDF's map images, which the Markdown export doesn't have); the stop heading is built the same place-then-name, two-line way in both (see "Stop label" above).
  - **Inconsistency found on request ("is the Markdown export consistent with the PDF one?")**: `poiParts()` (PDF) was missing the POI-level flags line (`booking-required`/`kid-friendly`, `itin.poi.popup.bookingRequired`/`itin.poi.popup.kidFriendly`) that `poiLines()` (Markdown, same section) already includes — a gap introduced when the PDF's POI rendering was written from scratch instead of reusing `poiLines()`'s field list. First fixed by adding the same flags line to `poiParts()` — then, on request, removed again specifically from the PDF (`poiParts()` no longer builds a `flags` array at all), while `poiLines()` (Markdown) keeps showing it. **This is now a deliberate, requested divergence between the two exports**, not an oversight — the PDF's POI entries are number/name/date + description only; Markdown's still include the booking-required/kid-friendly flags.
- **Page-break handling**: `ensureSpace(height)` adds a new page whenever the next block wouldn't fit above the bottom margin. Each stop's heading and its immediate content (type, dates, address, flags, booking) are built as a `{ text, size, bold, gap }` array (`parts`) *before* anything is drawn; `drawParts(parts)` measures the whole array's total height via `partHeight()` (which re-derives `doc.splitTextToSize()`'s line count at each part's font size — jsPDF's wrapping is a function of the currently-set font/size) and calls `ensureSpace()` **once** for that total, then draws every part. This moves the whole block to a fresh page together when it doesn't fit, rather than breaking mid-block and stranding the heading alone at the bottom of the previous page (a bug found in the field — a lone `ensureSpace(10)` before the heading only guaranteed the heading itself had room, not what followed it). The "Other points of interest" heading gets the same treatment, paired with its first entry.
  - **Bug found in the field — page 1 showing only the title and overview map, with most of the page left blank**: the stop's nested POIs were originally pushed into the *same* `parts` array as the heading/details (`pois.forEach(poi => parts.push(...poiParts(poi)))`) before calling `drawParts(parts)` once. `computeStoryGroups()`'s POI-to-stop assignment is data-driven and unbounded — a stop can have many POIs — so on a real itinerary where stop #1 had a large POI list, the *combined* block (heading + all its POIs) exceeded the space remaining after the title and overview map, and `drawParts()` correctly did its job: it moved the *entire* block to page 2, POIs included, because that's exactly what "keep this array together" means once the array contains the unbounded part too. Nothing else was scheduled for page 1, so it rendered as title + map + blank space. **Fix**: only the short, genuinely bounded part (heading, type, dates, address, flags, booking) goes through `drawParts()`; the POI list is drawn afterward, each POI free to flow across a page boundary on its own without stranding anything.
  - **Follow-up bug found in the field — a POI's own title split from its description across a page break**: the fix above initially drew each POI's parts via `poiParts(poi).forEach(p => addText(p.text, p))` — one independent `ensureSpace()` per part, so a POI's title (part 1) could land at the very bottom of a page while its description (part 2) started on the next one. That's a different unit than the stop-level bug: a *single* POI's title+description is always exactly 1–2 parts, genuinely bounded, unlike the unbounded per-stop POI list. **Fix**: `pois.forEach(poi => drawParts(poiParts(poi)))` — `drawParts()` per individual POI, keeping just that POI's own title+description together, while still letting different POIs independently fall on either side of a page break (matching the same "bounded unit gets the stranding guard, unbounded list doesn't" principle as the fix above, just one level down). Applied identically to the "Other points of interest" section's own per-POI loop.
- **OSM attribution — missing, then added**: OSM's tile usage policy requires visible attribution wherever the tiles are displayed, including on printed pages. The live map gets this for free from Leaflet's attribution control (`osmTileLayer()`'s `attribution` option); `renderStaticMapDataUrl()`'s OpenLayers map, however, is built with `controls: []` (deliberately, so OL's own attribution control doesn't end up baked into the composited PNG at some arbitrary corner/size chosen for on-screen use, not print) — and nothing replaced it, so every exported map image had no attribution at all, found on request rather than by testing. **Fixed**: `addMapImage()` now draws a small `"© OpenStreetMap contributors"` caption, right-aligned under every map image (overview and leg maps alike, since both go through this one function), in gray (`setTextColor(120)`, reset to black immediately after so it doesn't leak into subsequent text). Not run through `t()` — like `osmTileLayer()`'s own attribution string, this is fixed legal text mandated by OSM's license, not something that should vary by locale.
  - **Bug found in the field — the caption overlapped the bottom of the map image**: `doc.text()`'s `y` argument is the text's *baseline*, not its top edge; the caption was first placed only 1mm below the image, which isn't enough clearance for a 6pt font's ascenders (glyph tops extend upward from the baseline) — so the caption's own letters poked up into the map image. First fix widened the gap to 4mm, which cleared the overlap but (also found in the field) left a visibly oversized blank band between map and caption. **Settled on 2.5mm** — clears the ascenders without the empty-looking gap.
